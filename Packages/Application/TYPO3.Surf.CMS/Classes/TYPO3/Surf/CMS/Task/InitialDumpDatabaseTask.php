 ...
 */
class InitialDumpDatabaseTask extends Task {
	...
	public function execute(Node $node, Application $application, Deployment $deployment, array $options = array()) {
		$options = $application->getOption('contextConfiguration');
		$this->assertRequiredOptionsExist($options);

		$targetReleasePath = $application->getDeploymentPath();
		$lockfile = $targetReleasePath . '/.initialReleaseDone.lock';
		$username = isset($options['username']) ? $options['username'] . '@' : '';
		$hostname = $node->getHostname();
		$port = $node->hasOption('port') ? '-P ' . escapeshellarg($node->getOption('port')) : '';
		$commands[] = "if ssh {$port} {$username}{$hostname} \"[ ! -f ${lockfile} ] && touch ${lockfile}\"; then mysqldump -h {$options['sourceHost']} -u{$options['sourceUser']} -p{$options['sourcePassword']} {$options['sourceDatabase']} | ssh {$port} {$username}{$hostname} 'mysql -h {$options['targetHost']} -u{$options['targetUser']} -p{$options['targetPassword']} {$options['targetDatabase']}'; fi";
